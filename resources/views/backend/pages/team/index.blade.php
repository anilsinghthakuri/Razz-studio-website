                            ...
                            @foreach ($team_data as $item)
                                <div class="col-md-12 col-lg-6 col-xl-4">
                                    <div class="card mb-2 bg-gradient-dark">
                                        <img class="card-img-top" src="{{asset('backend/images/team/'.$item->image)}}"
                                            alt="Dist Photo 1">
                                        <div class="card-img-overlay d-flex flex-column justify-content-end">
                                            <h1 class=" text-danger bg-white ">{{Str::limit($item->name,15, '...')}}</h1>
                                            <h1 class=" text-danger bg-white ">{{Str::limit($item->designation,15, '...')}}</h1>
                                            ...
                                            <form action="{{route('team.destroy',$item->id)}}" method="post" id="form1">
                                                @csrf
                                            <a href="{{route('team.edit',$item->id)}}" class="text-white"><i class="fas fa-edit"
